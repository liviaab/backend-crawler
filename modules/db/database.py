...
def __select_parties_involved(cursor, process_number):
    query = 'SELECT pi.* FROM parties_involved as pi '\
                    'LEFT JOIN processes as p ON pi.process_id = p.id AND p.process_number = %s;'
...
def __select_movimentations(cursor, process_number):
    query = 'SELECT m.* FROM movimentations as m '\
            'LEFT JOIN processes as p ON m.process_id = p.id AND p.process_number = %s;'
    return __execute_query__(cursor, query, (process_number,))

def __format_process_info(info):
    insert_values = (info['Processo:'], info['Classe:'], info['Área:'], info['Assunto:'],
                    info['Distribuição:'], info['Juiz:'], info['Valor da ação:'],
                    datetime.now(), DEFAULT_COURT_ID)
...
def __insert_into_process(cursor, process_info):
    query = 'INSERT INTO processes(process_number, process_class, area, subject, '\
                    'distribution_date, judge, value, last_access, court_id) '\
...
def __insert_into_movimentations(cursor, process_id, changes):
    query = 'INSERT INTO movimentations(process_id, date, description) VALUES %s;'
